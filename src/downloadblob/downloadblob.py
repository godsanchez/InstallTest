...
# Ganymede API values
API_URL = "https://onefleetapi-davidpetika.azurewebsites.net"
...
CLIENT_ID = "9541a606-b37b-455f-aa70-9a6235cfb9b5"
SCOPE = "api://d491db24-e104-4eb5-846b-b9918ac34308/user_impersonation"
...
group.add_argument("--client-secret", type=str, help="Client secret for client-credentials auth mode.", default=None)
...
def get_ganymede_rest_token(client_id: str, tenant_id: str, scope: list[str]) -> str:
    ...
    Attempts managed identity, then client credentials, then interactive authentication.
    """

    token = _acquire_token_managed_identity(scope)

    if (token):
        return token
    else:
        token = _acquire_token_client_credentials(client_id, tenant_id, scope)

        if (token):
            return token
        else:
            token = _acquire_token_interactive(client_id, tenant_id, scope)

            if (token):
                return token
            else:
                raise RuntimeError("All authentication methods failed. No token obtained.")


def _acquire_token_managed_identity(scope: list[str]) -> str:
    """
    Acquires a token using Azure Managed Identity (for Azure ARC non-person accounts).
    """
    credential = ManagedIdentityCredential()

    # azure-identity uses .default suffix for scopes
    resource_scope = scope[0] if scope[0].endswith("/.default") else scope[0].rsplit("/", 1)[0] + "/.default"
...
def _acquire_token_client_credentials(client_id: str, tenant_id: str, scope: list[str]) -> str:
    """
    Acquires a token using client credentials (client secret).
    """
    ...
    client_scope = [s.rsplit("/", 1)[0] + "/.default" if not s.endswith("/.default") else s for s in scope]

    result = app.acquire_token_for_client(scopes=client_scope)
    ...
    if (isinstance(result, dict)):
        ...
        print("Client credentials authentication failed. No token obtained.")

    return str()


def _acquire_token_interactive(client_id: str, tenant_id: str, scope: list[str]) -> str:
    """
    Acquires a token using interactive (delegated user) authentication.
    """
    authority = f"https://login.microsoftonline.com/{tenant_id}"
    ...
    if (isinstance(result, dict)):
        ...
        print("Authentication failed. No token obtained.")
